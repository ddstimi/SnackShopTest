...
export default function HomePage() {
    ...
    const fetchProducts = async () => {
      if (fetchedRef.current == false) {
        try {
          ...
          setProducts(res);
          // console.log("Fetched products: " + res[0].name);
        } catch (err) {
  ...
  const handlePurchase = async () => {
    try {
      ...
      const errorMessage =
        err.response?.data?.error || "Ismeretlen hiba történt";
      toast.error(
        <>
          <div className="grid grid-cols-1">
            <strong className="">Hiba történt</strong>
            ...
            <button type="button" onClick={toggleCart}>
              <span
                className={`material-symbols-outlined font-semibold px-4 py-1 rounded-full transition relative inline-flex items-center p-3 text-sm text-center ${
                  scrolled
                    ? "bg-cyan-800 text-white hover:bg-cyan-800/50 hover:border-cyan-800 hover:border-px"
                    : "bg-white/90 text-cyan-800 hover:bg-white"
                }`}
              >
                shopping_cart
              </span>
              <div
                className={`absolute inline-flex items-center justify-center w-5 h-5 text-xs font-bold  ${
                  scrolled
                    ? "bg-white text-lorange hover:bg-white"
                    : "bg-lorange text-white hover:bg-lorange/70 hover:border-dorange hover:border-px"
                } rounded-full -top-2 right-16`}
              >
                {cartItems.length}
              </div>
            </button>
              ...
              <div className="absolute right-0 top-10 w-80 bg-white rounded-xl shadow-xl p-4 z-50">
                ...
                {cartItems.length === 0 ? (
                  <div className="space-y-3">
                    ...
                    <button
                      className="w-full bg-lorange text-white py-2 rounded hover:bg-lorange/80 transition disabled:opacity-50"
                      disabled
                    >
                      Vásárlás
                    </button>
                  ...
                  <div className="space-y-3">
                    {cartItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex justify-between items-center border-b-1 py-1 border-gray-100"
                      >
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="px-2 py-0 bg-gray-200 rounded"
                          >
                            -
                          </button>
                          <input
                        ...
                        <button
                          ...
                          className="text-lorange/80 hover:text-dorange"
                        >
                          ✕
                        </button>
                      </div>
                    ...
                    <button
                      className="w-full bg-lorange text-white py-2 rounded hover:bg-lorange/80 transition"
        ...
        <div className="relative z-10 flex flex-col justify-end h-full p-6">
          <div className="text-white text-center mb-16 md:mb-24">
            <h2 className="text-4xl md:text-6xl font-bold drop-shadow-lg">
              Világi ízek egy helyen
            </h2>
            <p className="text-lg mt-4 drop-shadow-md">
              Böngéssz a leghíresebb termékek között és válaszd ki, amelyik a
              legharapnivalóbb számodra!
            </p>
          </div>
        </div>
      </div>
