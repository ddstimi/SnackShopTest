          ...
          >
            <div className="w-1/2 flex flex-col lg:flex-row">
              <div className="w-1/2 hidden lg:flex items-center justify-center text-white text-center p-6">
                <div>
                  ...
                  <p className="mb-10">Még nincs fiókod? Csatlakozz és kóstolj bele a lehetőségekbe!</p>
                  <button
              ...
              <div className="w-full lg:w-1/2 flex items-center justify-center bg-white/80 p-8 z-10 gap-9">
                <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} className="w-full max-w-sm lg:space-y-6 space-y-2">
                  <h2 className="text-2xl font-bold">Bejelentkezés</h2>
                  <input type="text" required placeholder="Felhasználónév" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full p-2 border rounded" />
                  <input type="password" required value={password}  onChange={(e) => setPassword(e.target.value)} placeholder="Jelszó" className="w-full p-2 border rounded" />
                  <button type="submit" className="w-full border-dorange border-1 font-bold  rounded-lg text-white hover:bg-lorange/80 bg-lorange/100 p-2 mt-2 transform">Bejelentkezés</button>
                  ...
                  <div className="lg:invisible md:invisible visible flex flex-row flex-1 justify-center flex-wrap">
                    ...
                    <button type="button" onClick={() => setIsLogin(false)} className="px-10 py-2 border border-gray-400 rounded-lg hover:bg-gray-600/30 bg-gray-400/10 transition mt-4">Regisztráció</button>
                  </div>
            ...
            <div className="w-1/2 flex flex-col lg:flex-row">
              <div className="w-full lg:w-1/2 flex items-center justify-center bg-white/80 p-8 z-10 gap-6">
                <form className="w-full max-w-sm lg:space-y-6 space-y-2" onSubmit={(e) => { e.preventDefault(); handleRegister(); }}>
                  <h2 className="text-2xl font-bold">Regisztráció</h2>
                  <input type="text" placeholder="Felhasználónév" required value={username} onChange={(e) => setUsername(e.target.value)} className="w-full p-2 border rounded" />
                  <input type="password" placeholder="Jelszó" required value={password}  onChange={(e) => setPassword(e.target.value)} className="w-full p-2 border rounded" />
                  <button type="submit" className="w-full border-dorange border-1 font-bold  rounded-lg text-white hover:bg-lorange/80 bg-lorange/100 p-2 mt-2 transform">Regisztráció</button>
                   ...
                   <div className="lg:invisible md:invisible visible flex flex-row flex-1 justify-center flex-wrap">
                      ...
                      <button type="button" onClick={() => setIsLogin(true)} className="px-10 py-2 border relative align-middle border-gray-400 rounded-lg hover:bg-gray-600/30 bg-gray-400/10 transition mt-4">Bejelentkezés</button>
                    </div>
                </form>
              </div>
              <div className="w-1/2 hidden lg:flex items-center justify-center text-white text-center p-6">
                <div>
                  <h1 className="text-4xl font-extrabold mb-2">Ízleld meg a világot!</h1>
                  <p className="mb-10">Már van fiókod? Kattints a bejelentkezéshez.</p>
                  <button type="button"
